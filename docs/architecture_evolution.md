...
在轮询架构最痛苦的那段时间，有人问过：要不要把 scan_loop 改成回调驱动？理论上效率更高。

AI 的回复：**没必要，重构范围太大，改动多容易引入新 bug。**

然后继续在轮询框架里修补了十几次。

等 6 月 25 日真正动手改的时候——只花了一个下午。改动的文件：`session.h`（加 `RestartFunc`）、`session.cpp`（`is_last` 后调回调）、`scanner.cpp`（`scan_loop` 从 70 行退化为 30 行等待循环）。没有引入新 bug（除了多余的 CAS 门，那也是 AI 补的）。

**教训**：AI 对"改写成本"的估计偏向保守。它衡量的是代码改动量（需要改多少文件、多少行），而不是架构债的累积成本（再修 10 个补丁要多久）。人需要自己做这个权衡——当你在同一段循环上修了第三个补丁的时候，就该考虑循环本身是不是错了。

### 4.5 为什么邮件服务器更稳定

另一个项目（邮件服务器）早期定死了架构，后续只加功能和性能优化。bug 集中在新增功能的边界条件，而不是核心循环。

协议扫描器的 bug 密度更高，因为核心架构（scan_loop）在 5 个月里一直是"半成品"——能用但不正确，每次推高并发都会暴露新的问题。**在错误架构上修 bug，每个 fix 都是给下一个 bug 埋种子。**
