...
struct RtspProbeContext {
    ...
    RtspProbeContext(boost::asio::any_io_executor exec, Timeout t,
...
void RtspProtocol::async_probe(
    ...
    auto ctx = std::make_shared<RtspProbeContext>(std::move(exec), timeout, std::move(on_complete));
    ...
    ctx->timer.async_wait([ctx](const boost::system::error_code& ec) {
        if (!ec) ctx->finish_error("RTSP probe timed out");
    });
...
void RtspProtocol::parse_capabilities(const std::string&, ProtocolAttributes&) {
    // Inline in async_probe
}
